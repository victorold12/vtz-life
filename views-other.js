...
function viewConfig(m){
  ...
  <div class="two-col">
    ...
    <div class="grid" style="gap:16px">
      ...
      <div class="card">
        <b style="font-family:var(--display);font-size:15px;display:block;margin-bottom:6px">🎙 ElevenLabs — Voz da Biblioteca</b>
        <p style="color:var(--txt-3);font-size:12px;margin-bottom:12px">Voz premium para audiobooks na Biblioteca VTZ. Obtenha sua chave em <a href="https://elevenlabs.io" target="_blank" style="color:var(--accent-2)">elevenlabs.io</a></p>
        <div class="frow" style="margin-bottom:10px"><label class="fl">API Key</label><input class="input" id="elevenLabsKey" type="password" placeholder="sk_…" value="${esc(localStorage.getItem('vtz_elevenlabs_key')||'')}"></div>
        <div class="frow" style="margin-bottom:10px"><label class="fl">Voice ID</label><input class="input" id="elevenLabsVoiceId" placeholder="ID da voz (ver em elevenlabs.io/voices)" value="${esc(localStorage.getItem('vtz_elevenlabs_voice_id')||'')}"></div>
        <div style="display:flex;gap:9px"><button class="btn btn-primary" onclick="saveElevenLabsKey()">Salvar</button><button class="btn btn-ghost" onclick="testElevenLabsKey()">Testar</button></div>
      </div>
      <div class="card">
        ...
        <div style="display:flex;gap:9px"><button class="btn btn-primary" onclick="saveVoicePrefs()">Salvar voz</button><button class="btn btn-ghost" onclick="previewVoice()">▶ Testar agora</button></div>
      </div>
      <div class="card">
...
function saveElevenLabsKey(){const k=document.getElementById('elevenLabsKey')?.value.trim();const vid=document.getElementById('elevenLabsVoiceId')?.value.trim();if(k)localStorage.setItem('vtz_elevenlabs_key',k);if(vid)localStorage.setItem('vtz_elevenlabs_voice_id',vid);if(k||vid)toast('ElevenLabs configurado!','🎙')}
async function testElevenLabsKey(){toast('Testando…','⏳');try{const audio=await elevenLabsSpeak('Biblioteca VTZ ativa. Qualidade de voz premium ElevenLabs.');audio.onended=()=>toast('ElevenLabs OK!','✅')}catch(e){toast('Erro: '+e.message,'❌')}}
...
function previewVoice(){
  const v=document.getElementById('rvVoice')?.value||getVoicePrefs().voice;
  const r=parseFloat(document.getElementById('rvRate')?.value||getVoicePrefs().rate);
  const pi=parseFloat(document.getElementById('rvPitch')?.value||getVoicePrefs().pitch);
  if(window.responsiveVoice){responsiveVoice.cancel();responsiveVoice.speak('Olá Victor. JARVIS ativo e pronto para te ajudar.',v,{rate:r,pitch:pi,volume:1});}
  else speak('Olá Victor. JARVIS ativo e pronto para te ajudar.');
}
  ...
  if(!_isIOS&&window.responsiveVoice){
    try{
      ...
      responsiveVoice.speak(text,p.voice,{rate:p.rate,pitch:p.pitch,volume:1,
        onend:null,onerror:()=>speakBrowserFallback(text,p)});
      return;
    }catch(e){}
  }
